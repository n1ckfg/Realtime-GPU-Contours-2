...
namespace MPipeline.Custom_Data.PerMesh_Data {
    ...
    public class LineDrawingObject : MonoBehaviour {
        ...
        [Range(0f, 0.5f)] public float RotateX = .0f;
        [Range(0f, 1.0f)] public float RotateY = .0f;
        [Range(0f, 0.5f)] public float RotateZ = .0f;

        [Range(-0.05f, 0.05f)] public float TranslateX = .0f;
        [Range(-0.01f, 0.01f)] public float TranslateY = .0f;
        [Range(-0.5f, 0.5f)] public float TranslateZ = .0f;
        
        [Range(1000f, 10000f)] public float Period = 1000f;

        // Mesh Data Accessors for basic mesh properties
        public int TriangleCount => meshBufferSrc.TriangleCount;
        public int VertexCount => meshBufferSrc.VertexCount;

        // Cached mesh data, as data source for both *CPU & GPU* sides.
        public MeshBufferSource meshBufferSrc;

        // Scriptable Object on *GPU* side.
        public MeshDataGPU meshDataGPU;

        // Cameras that should render this LDO
        private List<Camera> _cameras = new List<Camera>();

        // LDMs bond with cameras
        private readonly List<LineDrawingMaster> _ldmList = new List<LineDrawingMaster>();
        ...
        public void Awake() {
            ...
            if (meshBufferSrc == null) {
                meshBufferSrc = PerMeshDataPreloadFactory.CreateData<MeshBufferSource>(new MeshDataSrcPreload(inputMesh));
            }

            if (_meshDataCpuPreload == null) {
                _meshDataCpuPreload = PerMeshDataPreloadFactory.CreateData<MeshDataCPUPreload>(new MeshDataCPUSrc(inputMesh, meshBufferSrc));
            }
            ...
            // 2) Init runtime data 
            _meshDataCpuRuntime = PerMeshDataRuntimeFactory.CreateData<MeshDataCPURuntime>(new MeshDataSrcRT(gameObject, meshBufferSrc, _cameras[0]));
            ...
            foreach (Camera cam in _cameras) {
                LineDrawingMaster ldm = PerCameraDataFactory.GetOrCreateSingleton<LineDrawingMaster>(cam);
            ...
            ContourExtractionPass.UpdatePerMeshData += (cam, cmd) => OnUpdatePerMeshData_ContourExtraction(cam, cmd);
        }
        ...
        public void OnDisable() {
            ...
            foreach (LineDrawingMaster ldm in _ldmList) {
                if (ldm != null)  ldm.Remove(this);

                // Output log info
                Debug.Log(gameObject + " cancelled registrition as LineDrawingObject from LineDrawingMaster.");
            }
        }
        ...
        public void Update() {
            ...
            gameObject.transform.Rotate(RotateX * turnaround, RotateY * turnaround, RotateZ * turnaround, Space.Self);

            gameObject.transform.Translate(new Vector3(turnaround * TranslateX, turnaround * TranslateY, turnaround * TranslateZ), Space.World);

            ++_currFrame;
        }
        
        private void OnUpdatePerMeshData_ContourExtraction(Camera cam, CommandBuffer cmd) {
            // Check if this mesh is bond with camera
            if (!_cameras.Contains(cam)) return;

            // -----------------------------------------
            // First time this delegate is called, load data form CPU into GPU.
            // Check if Per-Mesh Buffers have been initialized. If not, setup these buffers.
            if (!meshDataGPU.LoadedDeprecated) {
                meshDataGPU.LoadMeshBuffersCommand(_meshDataCpuPreload, cmd);
            }
